...
/*
Now, as we have used shared mutex, if a thread has acquired lock_shared then other threads can also acquire lock_shared.
But if it is noraml lock then no other thread can acquire this lock.

So, shared lock is used for readers as many reader threads can access the shared resource/critical section at a time
and normal lock which is exclusive is used for Writer
...
Reader-9 sees today is Tuesday
Reader-1 sees today is Tuesday
Reader-8 sees today is Tuesday
Reader-3 sees today is Tuesday
Reader-7 sees today is Tuesday
